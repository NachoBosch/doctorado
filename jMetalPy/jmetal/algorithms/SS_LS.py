...
class ScatterSearch(Algorithm[S, R]):
    ...
    def improve_solutions(self, solutions: List[S]) -> List[S]:
        improved_solutions = []
        for solution in solutions:
            mutated_solution = self.mutation.execute(solution)
            improved_solution = self.local_search(mutated_solution)
    ...
    def local_search(self, solution):
        ...
        for _ in range(local_search_iterations):
            ...
            if self.comparator.compare(neighbor, best_solution) < 0:
                best_solution = neighbor
        return best_solution

    def generate_neighbor(self, solution):
        neighbor = copy.deepcopy(solution)
        index = random.randint(0, len(neighbor.variables) - 1)
        neighbor.variables[index] = not neighbor.variables[index]
        return neighbor
