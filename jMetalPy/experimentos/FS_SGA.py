...
for a in alfa:
    for i in range(10):
        for model_name, model in zip(models_names[:1],models[:1]):
            # print(model_name)
            jobs = configure_experiment(problems={"FS_SGA": fsh.FeatureSelectionHD(data,a,model)},
                                        n_run=2)

            output_directory = make_dir(f"{os.getcwd()}/results/Resultados_SGA/cursoCE_final_{i}/",model_name,a)
